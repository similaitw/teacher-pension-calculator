...
function renderReport(data){
  ...
  if(data.incomeComparison){const x=data.incomeComparison,cross=x.crossoverAge?`${Number(x.crossoverAge).toFixed(1)} 歲`:'無／起點即領先',summary=document.createElement('p');summary.className='income-report-summary';summary.textContent=`年度收入比較｜${x.earlyAge} 歲減額月退 ${money(x.earlyMonthly)} 元／月｜延至 ${x.lateAge} 歲月退 ${money(x.lateMonthly)} 元／月｜延後期間工作收入 ${money(x.salaryUntilLate)} 元｜黃金交叉 ${cross}`;scenarios.append(summary)}

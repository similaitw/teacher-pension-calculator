...
function renderReport(data){
  ...
  const basic=q('basicData');addData(basic,'適用制度',data.systemLabel);addData(basic,'計算規則版本',data.rulesVersion||'舊版報表未記錄');addData(basic,'出生年月',rocText(data.input.birth));addData(basic,'正式到職',rocText(data.input.start));addData(basic,'預計退休',rocText(data.input.retire));addData(basic,'初任學歷',data.input.initialEducation);addData(basic,'目前薪級',`${data.input.currentPoint}（${Number(data.input.currentSalary).toLocaleString('zh-TW')} 元）`);addData(basic,'每月固定薪給',`${money(data.input.workIncome||data.input.currentSalary)} 元`);addData(basic,'年度獎金假設',`年終 ${Number(data.input.yearEndMonths||0)}＋考核 ${Number(data.input.performanceMonths||0)} 個月`);
  ...
  if(data.incomeComparison){const x=data.incomeComparison,includesWork=x.includeWorkIncome!==false,cross=x.crossoverAge?`${Number(x.crossoverAge).toFixed(1)} 歲`:'無／起點即領先',summary=document.createElement('p');summary.className='income-report-summary';summary.textContent=`年度收入比較｜口徑：${includesWork?'含工作收入':'只比退休金'}｜${x.earlyAge} 歲減額月退 ${money(x.earlyMonthly)} 元／月｜延至 ${x.lateAge} 歲月退 ${money(x.lateMonthly)} 元／月${includesWork?`｜延後期間工作收入 ${money(x.salaryUntilLate)} 元`:''}｜黃金交叉 ${cross}`;scenarios.append(summary)}

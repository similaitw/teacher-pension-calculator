...
function renderReport(data){
  ...
  const scenarios=q('scenarioData');(data.comparison||[]).forEach(x=>{const row=document.createElement('div');row.className='scenario-report-row';const values=data.system==='fund'?[x.label,`${Number(x.lump).toLocaleString('zh-TW')} 元`,`${Number(x.monthly).toLocaleString('zh-TW')} 元／月`,`${Number(x.at75).toLocaleString('zh-TW')} 元`]:[x.label,`${Number(x.lump).toLocaleString('zh-TW')} 元`,'依核定月領方案','—'];values.forEach(v=>{const span=document.createElement('span');span.textContent=v;row.append(span)});scenarios.append(row)});
